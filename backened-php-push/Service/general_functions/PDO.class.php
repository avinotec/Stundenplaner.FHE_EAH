...
class DB
{
	...
	private function Connect()
	{
		try {
			...
			//For PHP 5.3.6 or lower
			$this->pdo->setAttribute(PDO::MYSQL_ATTR_INIT_COMMAND, 'SET NAMES utf8; SET time_zone = TIMEZONE');
	...
	private function ExceptionLog(PDOException $e, $sql = "", $method = '', $parameters = array())
	{
		...
		if (
			...
			if (($this->pdo === null || !$this->inTransaction()) && php_sapi_name() !== "cli") {
				...
				if(array_key_exists("PDOError", get_defined_constants(true)['user']) AND get_defined_constants(true)['user']["PDOError"] == 1 ){
					echo $exception;
				}
				//echo $exception;
				exit();
